...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed){
        for(String k:  drivable_map.keySet()){
            if(drivable_map.get(k).getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public List getTradable(){
        List<Drivable> lst = new ArrayList<Drivable>();
        for(String k:  drivable_map.keySet()){
            if(drivable_map.get(k) instanceof Tradable){
                lst.add(drivable_map.get(k));
            }
        }
        return lst;
    }
